...
public class Scheduler {
  ...
  public static void run(NachosThread nextThread) {
    ...
    if (Nachos.USER_PROGRAM) {  // ignore until running user programs 
      if (oldThread.getSpace() != null) {// if this thread runs a user program,
        oldThread.saveUserState(); // save the user's CPU registers
	oldThread.getSpace().saveState();
      }
    }
    ...
    if (Nachos.USER_PROGRAM) {
      if (oldThread.getSpace() != null) {// if there is an address space
	oldThread.restoreUserState();     // to restore, do it.
	oldThread.getSpace().restoreState();
      }
    }
  }
